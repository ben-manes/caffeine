...
@SuppressWarnings({"AlmostJavadoc", "ClassNamedLikeTypeParameter", "EmptyCatch",
  "IdentifierName", "InvalidBlockTag", "rawtypes", "StreamOfArray"})
public class ConcurrentContainsKeyTest {
    ...
    public void testContainsKey() {
        ...
        ConcurrentMap<Object, Object> bounded = bounded();
        Stream.of(content).forEach(x -> bounded.put(x, x));
        test(content, bounded);

        ConcurrentMap<Object, Object> unbounded = unbounded();
        Stream.of(content).forEach(x -> unbounded.put(x, x));
        test(content, unbounded);
    }

    private static void test(X[] content, ConcurrentMap<Object, Object> m) {
        for (int i = 0; i < I; i++) {
            testOnce(content, m);
        }
    }
    ...
    private static void testOnce(Object[] content, ConcurrentMap<Object, Object> m) {
