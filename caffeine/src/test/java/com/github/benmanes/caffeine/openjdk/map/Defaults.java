...
public class Defaults {
     ...
     */
    @Test(dataProvider = "ImplementsConcurrentMap")
    public void testConcurrentMap_computeIfAbsent_racy(String name, ConcurrentMap<Long, Long> map) {
        final Long two = 2L;
     ...
     */
    @Test(dataProvider = "ImplementsConcurrentMap")
    public void testConcurrentMap_computeIfPresent_racy(String name, ConcurrentMap<Long, Long> map) {
    ...
    @Test(dataProvider = "ImplementsConcurrentMap")
    public void testConcurrentMap_compute_simple(String name, ConcurrentMap<Long, Long> map) {
     ...
     */
    @Test(dataProvider = "ImplementsConcurrentMap")
    public void testConcurrentMap_compute_racy(String name, ConcurrentMap<Long, Long> map) {
        final AtomicBoolean b = new AtomicBoolean(true);
        final Long two = 2L;
        BiFunction<Long,Long,Long> f, g;
     ...
     */
    @Test(dataProvider = "ImplementsConcurrentMap")
    public void testConcurrentMap_merge_racy(String name, ConcurrentMap<Long, Long> map) {
        final AtomicBoolean b = new AtomicBoolean(true);
        final Long two = 2L;
        BiFunction<Long,Long,Long> f, g;
    ...
    @DataProvider(name = "ImplementsConcurrentMap", parallel = true)
    public static Iterator<Object[]> implementsConcurrentMap() {
        return makeImplementsConcurrentMap().iterator();
    }
...
//        all.addAll(makeRWMaps(true,true));
    ...
    private static Collection<Object[]> makeRWNoNullsMaps() {
        Supplier<Map<IntegerEnum, String>> bounded = () -> {
            Cache<IntegerEnum, String> cache = Caffeine.newBuilder()
                .expireAfterWrite(Duration.ofNanos(Long.MAX_VALUE))
                .maximumSize(Long.MAX_VALUE)
                .build();
            return cache.asMap();
        };
        Supplier<Map<IntegerEnum, String>> unbounded = () -> {
          Cache<IntegerEnum, String> cache = Caffeine.newBuilder().build();
          return cache.asMap();
        };
        return Arrays.asList(
...
//            new Object[]{"ImplementsConcurrentMap", makeMap(ImplementsConcurrentMap::new, false, false)}
    ...
    private static Collection<Object[]> makeImplementsConcurrentMap() {
      var bounded = Caffeine.newBuilder()
          .expireAfterWrite(Duration.ofNanos(Long.MAX_VALUE))
          .maximumSize(Long.MAX_VALUE)
          .build();
      var unbounded = Caffeine.newBuilder().build();
      return Arrays.asList(
          // null key and value hostile
          new Object[]{"Bounded", new ImplementsConcurrentMap(bounded.asMap())},
          new Object[]{"Unbounded", new ImplementsConcurrentMap(unbounded.asMap())});
      }
    ...
    public static class ImplementsConcurrentMap<K,V> extends ExtendsAbstractMap<ConcurrentMap<K,V>, K, V> implements ConcurrentMap<K,V> {
        public ImplementsConcurrentMap(ConcurrentMap<K,V> delegate) { super(delegate); }
