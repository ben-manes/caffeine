...
@SuppressWarnings({"AlmostJavadoc", "MultiVariableDeclaration", "NonAtomicVolatileUpdate",
    "NonFinalStaticField", "rawtypes", "serial", "SystemOut", "unchecked", "unused",
    "UnnecessaryFinal", "UnusedNestedClass", "UnusedVariable"})
public class BiggernYours {
    ...
    static ColFrobber adder(final int i) {
        return new ColFrobber() {@Override
        void frob(Collection c) { c.add(i); }};
    }
    ...
    static MapFrobber putter(final int k, final int v) {
        return new MapFrobber() {@Override
        void frob(Map m) { m.put(k,v); }};
    }
    ...
    static void arrayEqual(Object[] x, Object[] y) {
        if (x == null ? y == null : Arrays.equals(x, y)) {
          pass();
        } else {
          fail(Arrays.toString(x) + " not equal to " + Arrays.toString(y));
        }}
