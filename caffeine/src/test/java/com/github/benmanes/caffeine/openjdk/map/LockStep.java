...
@SuppressWarnings({"MultiVariableDeclaration", "NonAtomicVolatileUpdate", "NonAtomicVolatileUpdate",
    "rawtypes", "RedundantStringConversion", "SystemOut", "unchecked", "UnnecessaryFinal"})
public class LockStep {
    ...
    void remove(Map m, Iterator it) {
        ...
        it.remove();
        if (m.size() != size-1) {
          throw new Error(String.format(US, "Incorrect size!%nmap=%s, size=%d%n",
                                        m.toString(), m.size()));
        }
    }

    void test(Supplier<Map> supplier) {
...
//                Collections.synchronizedNavigableMap(new TreeMap())
    ...
    void check(boolean cond) {if (cond) {
      pass();
    } else {
      fail();
    }}
