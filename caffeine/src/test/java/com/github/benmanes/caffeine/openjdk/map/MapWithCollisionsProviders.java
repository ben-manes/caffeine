...
@SuppressWarnings({"CanIgnoreReturnValueSuggester", "DirectReturn", "DoubleBraceInitialization",
    "MixedArrayDimensions", "PreferredInterfaceType", "serial", "UnusedVariable", "YodaCondition"})
public class MapWithCollisionsProviders {
    ...
    private static <T> Supplier<Map<T, T>> createMap(Supplier<Map<T, T>> supplier, T[] keys) {
        return () -> fillMap(supplier.get(), keys);
    }

    private static <T> Object[] createCase(String desc,
        Supplier<Map<T, T>> supplier, T[] keys, T val) {
        return new Object[]{desc, createMap(supplier, keys), val};
    }
...
//                             new LinkedHashMap<>(), keys, val));
        ...
        cases.add(createCase("Bounded with " + desc, () ->
            Caffeine.newBuilder()
                .expireAfterWrite(Duration.ofNanos(Long.MAX_VALUE))
                .maximumSize(Long.MAX_VALUE)
                .build().asMap(),
            keys, val));
        cases.add(createCase("Unbounded with " + desc, () ->
            Caffeine.newBuilder().build().asMap(), keys, val));
        return cases;
    }
