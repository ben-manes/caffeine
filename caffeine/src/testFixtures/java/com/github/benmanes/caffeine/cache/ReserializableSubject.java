...
import static com.github.benmanes.caffeine.cache.AsyncCacheSubject.asyncCache;
...
public final class ReserializableSubject extends Subject {
  ...
  private void checkEviction(
    ...
    check("isWeighted()").that(copy.isWeighted).isEqualTo(original.isWeighted);
  ...
  private static Weigher<?, ?> unwrapWeigher(@Var Weigher<?, ?> weigher) {
    for (;;) {
  ...
  @SuppressWarnings("PMD.AvoidReassigningParameters")
  private static Expiry<?, ?> unwrapExpiry(@Var Expiry<?, ?> expiry) {
    for (;;) {
