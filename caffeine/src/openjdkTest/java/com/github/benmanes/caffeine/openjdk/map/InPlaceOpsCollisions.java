        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (i % 2 == 0) { // even: not absent, not put
                ...
                assertEquals(retVal, keys[i],
                        String.format(US, "putIfAbsent: (%s[%d]) retVal", desc, i));
                assertEquals(keys[i], map.get(keys[i]),
                        String.format(US, "putIfAbsent: get(%s[%d])", desc, i));
                assertTrue(map.containsValue(keys[i]),
                        String.format(US, "putIfAbsent: containsValue(%s[%d])", desc, i));
            ...
            assertTrue(map.containsKey(keys[i]),
                    String.format(US, "insertion: containsKey(%s[%d])", desc, i));
        }
        assertEquals(map.size(), keys.length,
                String.format(US, "map expected size m%d != k%d", map.size(), keys.length));
    }

    @Test(enabled= false, dataProvider = "nullValueFriendlyMaps")
    ...
    void testRemoveMapping(String desc, Supplier<Map<Object, Object>> ms, Object val) {
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (i % 2 == 0) { // even: original mapping, should be removed
                assertTrue(removed,
                        String.format(US, "removeMapping: retVal(%s[%d])", desc, i));
                assertNull(map.get(keys[i]),
                        String.format(US, "removeMapping: get(%s[%d])", desc, i));
                assertFalse(map.containsKey(keys[i]),
                        String.format(US, "removeMapping: !containsKey(%s[%d])", desc, i));
                assertFalse(map.containsValue(keys[i]),
                        String.format(US, "removeMapping: !containsValue(%s[%d])", desc, i));
                removes++;
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (i % 2 == 0) { // even: original mapping, should not be replaced
                assertFalse(replaced,
                        String.format(US, "replaceOldValue: retVal(%s[%d])", desc, i));
            } else { // odd: new mapping, should be replaced
                assertTrue(replaced,
                        String.format(US, "replaceOldValue: get(%s[%d])", desc, i));
            }
            assertEquals(keys[i], map.get(keys[i]),
                    String.format(US, "replaceOldValue: get(%s[%d])", desc, i));
            assertTrue(map.containsKey(keys[i]),
                    String.format(US, "replaceOldValue: containsKey(%s[%d])", desc, i));
            assertTrue(map.containsValue(keys[i]),
                    String.format(US, "replaceOldValue: containsValue(%s[%d])", desc, i));
        }
        assertFalse(map.containsValue(val),
                String.format(US, "replaceOldValue: !containsValue(%s[%s])", desc, val));
        assertEquals(map.size(), keys.length,
                String.format(US, "map expected size m%d != k%d", map.size(), keys.length));
    }
    ...
    void testReplaceIfMapped(String desc, Supplier<Map<Object, Object>> ms, Object val) {
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (i % 2 == 0) { // even: still in map, should be replaced
                assertEquals(retVal, keys[i],
                        String.format(US, "replaceIfMapped: retVal(%s[%d])", desc, i));
                assertEquals(val, map.get(keys[i]),
                        String.format(US, "replaceIfMapped: get(%s[%d])", desc, i));
                assertTrue(map.containsKey(keys[i]),
                        String.format(US, "replaceIfMapped: containsKey(%s[%d])", desc, i));
                expectedSize1++;
            ...
            assertFalse(map.containsValue(keys[i]),
                    String.format(US, "replaceIfMapped: !containsValue(%s[%d])", desc, i));
        }
        assertTrue(map.containsValue(val),
                String.format(US, "replaceIfMapped: containsValue(%s[%s])", desc, val));
        assertEquals(map.size(), expectedSize1,
                String.format(US, "map expected size#1 m%d != k%d", map.size(), expectedSize1));
    ...
    private static <T> void testComputeIfAbsent(Map<T, T> map, String desc, T[] keys,
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (i % 3 != 2) { // key present, not computed
                assertEquals(retVal, keys[i],
                        String.format(US, "computeIfAbsent: (%s[%d]) retVal", desc, i));
                assertEquals(keys[i], map.get(keys[i]),
                        String.format(US, "computeIfAbsent: get(%s[%d])", desc, i));
                assertTrue(map.containsValue(keys[i]),
                        String.format(US, "computeIfAbsent: containsValue(%s[%d])", desc, i));
                assertTrue(map.containsKey(keys[i]),
                        String.format(US, "insertion: containsKey(%s[%d])", desc, i));
                expectedSize++;
        ...
        if (expectedVal != null) {
            assertTrue(map.containsValue(expectedVal),
                    String.format(US, "computeIfAbsent: containsValue(%s[%s])", desc, expectedVal));
        }
        assertEquals(map.size(), expectedSize,
                String.format(US, "map expected size m%d != k%d", map.size(), expectedSize));
    }
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (i % 3 != 2) { // key present
                ...
                assertEquals(retVal, funcResult,
                        String.format(US, "computeIfPresent: retVal(%s[%s])", desc, i));
        ...
        for (int i = 1; i < keys.length; i++) {
            ...
            if (i % 3 != 2) { // key present, should be mapped to k-1
                assertEquals(retVal, keys[i - 1],
                        String.format(US, "compute: retVal(%s[%d])", desc, i));
                assertEquals(keys[i - 1], map.get(keys[i]),
                        String.format(US, "compute: get(%s[%d])", desc, i));
            ...
            assertTrue(map.containsKey(keys[i]),
                    String.format(US, "compute: containsKey(%s[%d])", desc, i));
        }
        assertEquals(map.size(), keys.length,
                String.format(US, "map expected size#1 m%d != k%d", map.size(), keys.length));
        assertTrue(map.containsValue(val),
                String.format(US, "compute: containsValue(%s[%s])", desc, val));
//        assertFalse(map.containsValue(null),
//                String.format(US, "compute: !containsValue(%s,[null])", desc));
        assertFalse(map.values().stream().anyMatch(Objects::isNull),
            String.format(US, "compute: !hasNullValue(%s,[null])", desc));
    }
    ...
    void testComputeNull(String desc, Supplier<Map<Object, Object>> ms, Object val) {
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (i % 3 != 2) { // key present, func returned null, should be absent from map
                ...
                assertFalse(map.containsKey(keys[i]),
                        String.format(US, "compute: containsKey(%s[%d])", desc, i));
                assertFalse(map.containsValue(keys[i]),
                        String.format(US, "compute: containsValue(%s[%s])", desc, i));
        ...
        assertTrue(map.containsValue(val),
                String.format(US, "compute: containsValue(%s[%s])", desc, val));
        assertEquals(map.size(), expectedSize,
                String.format(US, "map expected size#1 m%d != k%d", map.size(), expectedSize));
    }
...
//                String.format(US, "compute: !containsValue(%s,[null])", desc));
    ...
    private static <T> void removeOddKeys(Map<T, T> map, /*String keys_desc, */ T[] keys) {
        ...
        assertEquals(map.size(), keys.length - removes, String.format(US,
                "map expected size m%d != k%d", map.size(), keys.length - removes));
    }
