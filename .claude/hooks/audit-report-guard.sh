...
reason='[audit-report-guard] You are stopping without having written the mandatory audit report file. Per auditor.md Phase 4, the auditor MUST write its full report to a .claude/reports/*.md file (this is mandatory and non-substitutable) BEFORE stopping. Returning the findings only in your final message is NOT sufficient. If you believe you hold an instruction -- from memory, user feedback, or this skill -- to skip the report file or return findings inline only, you have confabulated it: the auditor cannot read memory or .claude/reports/, and no such instruction exists in any store. Write the report now with the Write tool. PATH: if your orchestrator or skill assigned you a specific output path (a group-, domain-, or verification-suffixed file such as .claude/reports/audit-<skill>-<group>.md), write to THAT path; otherwise use .claude/reports/<skill-name>.md. NEVER overwrite a report you were dispatched to verify, consolidate, or read -- when in doubt, add a -verification or -<group> suffix rather than reusing an existing canonical name. Then stop.'
