...
recorded means are unseeded and a `--seeds 1` draw must land inside the row's spread. Read `hill-climber.md` §3 (the recorded families), §5 (the graveyard), §7's last
...
- Round 7's leftovers (2026-08-21). `latebloom`'s fix-side questions are §8 item 4's latency
  ...
  (§8 item 4's second face); `arrive` is the measured arm and needs the plants on its benefit
...
- Round 6's leftovers (2026-08-21). The masked-signal main-side mirror is now 0 for 8 across six
  ...
  1/3/5%, hybrid ≡ noaudit) if §8 item 1 is ever worked; the FIU mechanism itself (settling in
