...
`/climber-gate` and directions for `hill-climber.md` §8. Run it when a new model arrives, after a
...
- a class token from the table (`structural`, `wrong-equilibrium`, `slow-convergence`,
  ...
  Use it when the record already points at a class (`hill-climber.md` §8) rather than asking what
  is broken.
...
- The ceiling is the best **static** window, not an upper bound: an adaptive window can beat every
  ...
  beating both. Before crediting adaptivity, re-sweep densely around the peak with `run.py`'s
  `curve(trace, size, fmt, windows=[...])` from a Python one-liner and read the result by eye
  (`regret.py` caches the sweep in `<trace>.anchors.<size>.json` and has no window-list flag; a
  `--windows` option is the follow-up if this recurs).
...
- **Setup for trajectories and arms**: `git worktree add --detach <workspace>/tree HEAD`,
  `python3 .claude/skills/climber-gate/harness.py apply <workspace>/tree` (then `verify`; 38/38),
...
`dumps/`, `data/`. Report: `.local/audits/<model>/audit-regret.md` (`rules/audit-output.md`).
...
python3 $G/harness.py apply $PWD/tree && python3 $G/harness.py verify $PWD/tree      # 38/38
...
gate's generation block and pick a low-spread row (`straywall2_8192_d050`, `demoflood_8192`), not a
lottery cell, since the recorded means are unseeded and a `--seeds 1` draw must land inside the
row's spread. Read `hill-climber.md` §3 (the recorded families), §5 (the graveyard), §7's last
entries and §8 before proposing anything, and keep the §3 family list and the gate table at hand
for step 3.
...
- The **blind lane** gets `WindowClimber.java`, `hill-climber.md` §2 and §4 (the machine), the
  ...
  from.
- The **sighted lane** additionally gets §3, the gate table, §5 and §8's "do not reopen" list,
...
to `hill-climber.md` §3 (a family) or §7 (a dated entry) with the class named; open directions go
to §8. Write the report; leave the ledger current.
...
- The proposal lane on the session model (Fable) stalled in round 2 (four consecutive 96,000-token
  ...
  rule above now puts both lanes on Opus with a ~20-minute box. Round 3 ran the third-lane shape
  (three or four specs, each written to disk the moment it is designed, aimed at named lines) on
  Fable: three specs and predictions in nine minutes, done before the screen; none survived the
  screen (`f_ghostlure` recorded, `f_cornerloop` peak-at-edge, `f_crestshelf` clean), so the shape
  works and its yield is a fair sample of one.
- `hazefloor` (round 3): §8 item 4's residual isolated on a valid interior-peak terrain (the
  calibration audit crosses and parks by s24, and the corner audit's crash, the C2 discard,
  density's slide and the floor's failing ×1 walks are the rest of the run). A sentinel row needs an
  x4 run at fixed seeds (the 80-sample mean is a reach lottery and 160 samples show three shapes),
  per-seed bars, a 60/65% re-sweep (`--windows`), and ideally an arm with the crash-undo discard
  disabled to say whether the 9pp is the discard or the corner audit; the workspace holds the specs
  and dumps.
...
- Do not rebuild §5's graveyard, and do not reopen §8's "do not reopen" list without a new
